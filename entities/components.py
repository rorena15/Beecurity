...
class BaseComponent:
    """모든 컴포넌트의 부모 클래스입니다."""
    def update(self, tick: int, *args, **kwargs) -> None:
        pass
...
class LayingWorkerComponent(BaseComponent):
    ...
    def produce_dummy_node(self) -> str:
        """승인되지 않은 더미 노드(수컷벌)만 생성하여 시스템 자원을 고갈시킵니다."""
        if self.is_active:
            self.drone_egg_count += 1
            return "UNAUTHORIZED_DRONE_NODE"
        return "NONE"

    def update(self, tick: int, current_pheromone: float = 100.0, *args, **kwargs) -> None:
        # 페로몬(정책 신호)이 임계치 이상으로 회복되면 권한 드리프트 중단
